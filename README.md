...
- A GitHub Personal Access Token — [github.com/settings/tokens](https://github.com/settings/tokens) *(needed to avoid rate limits even for public repos)*
...
> **Tip:** Run `python test_env.py` to verify your Anthropic and LlamaCloud keys are loaded correctly.

#### Getting a GitHub Personal Access Token
1. Go to GitHub → profile photo → **Settings**
2. Scroll to **Developer settings** → **Personal access tokens** → **Tokens (classic)**
3. Click **Generate new token (classic)**
4. Tick **public_repo** (for public repos) or **repo** (for private repos)
5. Copy the token and paste it into `.env` as `GITHUB_TOKEN`
...
Each document is indexed with metadata: **semester**, **subject**, and **filename** — so the chatbot knows where each answer comes from.
...
When new notes are pushed to the GitHub repo, click **🔄 Refresh notes from GitHub** in the sidebar. This clears the cache and re-crawls GitHub.
